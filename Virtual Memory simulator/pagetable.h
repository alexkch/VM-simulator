...
/* The lru_node is used in the LRU algorithm as a node in the linked list
 * that keeps track of recently used frames
 * 
 * the lru_node keeps track of a frame number to the physical frame, and 
 * a pointer to another lru_node
 */
typedef struct ___lru_node_t {
	unsigned int frame; 
	struct ___lru_node_t * next;
} lru_node;
...
// Helper functions used by the LRU algorithm
